...
from board6x6 import hash_board_red_perspective, get_score
from utils import *
import operator
...
NAIVE_GIVEN_RANDOM = 0.6
END_GAME_REWARD = 50
LOST_GAME_PENALTY = -50


def flip_move(move, grid_len):
    grid_len -= 1
    flipped_move = []
    for m in move:
        flipped_move.append((grid_len - m[0], grid_len - m[1]))
    return tuple(flipped_move)


class ComputerPlayerRed():
    ...
    def choose_turn(self, turns, board):
        if hash_board_red_perspective(board.grid) in self.q_table and uniform(0, 1) > RANDOM_CHOICE_CHANCE:
            sorted_actions = sorted(self.q_table[hash_board_red_perspective(
                board.grid)].items(), key=operator.itemgetter(1))
            t = flip_move(eval(sorted_actions.pop()[0]), len(board.grid))
            return t
    ...
    def play(self, board, silent=False):
        if self.previous_state:
            if hash_board_red_perspective(board.grid) in self.q_table:
                # with open("q_update.txt", "a") as f:
                #     f.write("old_q " + str(self.q_table[self.previous_state][self.previous_action]))
                #     f.write("\nmax " + str(max([self.q_table[hash_board_red_perspective(board.grid)][a]
                #             for a in self.q_table[hash_board_red_perspective(board.grid)]])))
                #     f.write("\ndifference " + str(max([self.q_table[hash_board_red_perspective(board.grid)][a]
                #             for a in self.q_table[hash_board_red_perspective(board.grid)]])
                ...
                self.q_table[self.previous_state][self.previous_action] += (LEARNING_RATE * ((get_score(board.grid, board.turn) - get_score(self.previous_grid, board.turn)) - 0.1 +
                                                                                             DISCOUNT * max([self.q_table[hash_board_red_perspective(board.grid)][a]
                                                                                                             for a in self.q_table[hash_board_red_perspective(board.grid)]])
                                                                                             - self.q_table[self.previous_state][self.previous_action]))
        ...
        self.previous_state = hash_board_red_perspective(board.grid)
        self.previous_action = str(flip_move(t, len(board.grid)))
