...
def hash_board_red_perspective(grid):
    ...
    for i in range(0, len(grid)):
        ...
        if i % 2 != 0:
            j -= 1
        while j >= 0:
            val = grid[len(grid) - 1 - i][j]
            if val != Type.empty:
                if val >= Type.black:
                    val -= 2
                else:
                    val += 2
            l.append(str(bin(val))[2:].zfill(3))
            j -= 2
    return base36encode(int("".join(l), 2))
...
class Board6x6:
    ...
    def __init__(self):
        self.grid = [[Type.empty for i in range(BOARD_SIZE)] for j in range(BOARD_SIZE)]
    ...
    def _is_valid_move(self, grid, origin, destination):
        ...
        if not (0 <= ori_r <= BOARD_SIZE - 1):
            return False
        if not (0 <= ori_c <= BOARD_SIZE - 1):
            return False
        if not (0 <= dest_r <= BOARD_SIZE - 1):
            return False
        if not (0 <= dest_c <= BOARD_SIZE - 1):
            return False
    ...
    def _should_crown(self, position):
        ...
        if self.grid[pos_r][pos_c] == Type.red and pos_r == 0:
            return True
        if self.grid[pos_r][pos_c] == Type.black and pos_r ==  BOARD_SIZE - 1:
            return True
        return False
