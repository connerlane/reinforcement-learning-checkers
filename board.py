...
def hash_board_red_perspective(grid):
    l = []
    for i in range(0, len(grid)):
        j = len(grid) - 1
        if i % 2 == 0:
            j -= 1
        while j >= 0:
            l.append(str(bin(grid[len(grid) - 1 - i][j]))[2:].zfill(3))
            j -= 2
    return base36encode(int("".join(l), 2))
...
class Board:
    ...
    def __init__(self):  # hard coding size to 8x8 for now...
        self.grid = [[Type.empty for i in range(8)] for j in range(8)]
    ...
    def _should_crown(self, position):
        ...
        if self.grid[pos_r][pos_c] == Type.red and pos_r == 0:
            return True
        if self.grid[pos_r][pos_c] == Type.black and pos_r == 7:
            return True
        return False
    ...
    def _initialize_board(self):
        ...
        for i in range(0, len(self.grid)):
            if i % 2 == 0:
                self.grid[2][i] = Type.black
            else:
                self.grid[2][i] = Type.illegal

        for i in range(3, 5):
            for j in range(0, len(self.grid)):
                if i % 2 == 1 and j % 2 == 0:
                    self.grid[i][j] = Type.illegal
                if i % 2 == 0 and j % 2 == 1:
                    self.grid[i][j] = Type.illegal

        for i in range(0, len(self.grid)):  # place initial red pieces
            if i % 2 == 1:
                self.grid[5][i] = Type.red
            else:
                self.grid[5][i] = Type.illegal
        for i in range(0, len(self.grid)):
            if i % 2 == 0:
                self.grid[6][i] = Type.red
            else:
                self.grid[6][i] = Type.illegal
        for i in range(0, len(self.grid)):
            if i % 2 == 1:
                self.grid[7][i] = Type.red
            else:
                self.grid[7][i] = Type.illegal
